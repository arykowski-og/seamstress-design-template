export const dataFormatters = {
  ...
  trend: (current: number, previous: number): {
    ...
    if (Math.abs(change) < threshold) {
      return { direction: 'stable', arrow: '→', color: '#gray' };
    } else if (change > 0) {
      return { direction: 'up', arrow: '↑', color: '#00A859' };
    } else {
      return { direction: 'down', arrow: '↓', color: '#FF6B6B' };
    }
  },
  ...
  riskLevel: (variance: number, threshold: number = 10): {
    ...
    if (absVariance < threshold * 0.5) {
      return { level: 'low', color: '#00A859', indicator: '🟢' };
    } else if (absVariance < threshold) {
      return { level: 'medium', color: '#FFD93D', indicator: '🟡' };
    } else if (absVariance < threshold * 2) {
      return { level: 'high', color: '#FF6B6B', indicator: '🔴' };
    } else {
