 ...
 *
 * Functions to extract and work with design tokens from capitalDesignTokens
 */
...
/**
 * Extracts all color tokens from capitalDesignTokens
 */
export function extractColorTokens(): ColorToken[] {
  const tokens: ColorToken[] = [];

  // Extract from foundations.colors
  if (capitalDesignTokens.foundations?.colors) {
    Object.entries(capitalDesignTokens.foundations.colors).forEach(([key, value]) => {
  ...
  if (capitalDesignTokens.semanticColors) {
    Object.entries(capitalDesignTokens.semanticColors).forEach(([category, subcategories]) => {
