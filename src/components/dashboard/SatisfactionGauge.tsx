...
const SatisfactionGauge: React.FC<SatisfactionGaugeProps> = ({
  ...
  // Determine color based on zones using theme data viz colors
  const getColor = () => {
    if (clampedValue >= colorZones.green[0] && clampedValue <= colorZones.green[1]) {
      return capitalDesignTokens.semanticColors.dataVisualization.sequence800; // Teal green
    }
    if (clampedValue >= colorZones.yellow[0] && clampedValue <= colorZones.yellow[1]) {
      return capitalDesignTokens.semanticColors.dataVisualization.sequence200; // Coral/salmon
    }
    return capitalDesignTokens.semanticColors.dataVisualization.sequence200; // Coral for low values
  };
            ...
            <Stack direction="row" alignItems="center" spacing={0.5}>
              <Box
                sx={{
                  ...
                  height: 12,
                  borderRadius: '50%',
                  bgcolor: capitalDesignTokens.semanticColors.dataVisualization.sequence200,
                }}
              />
            ...
            <Stack direction="row" alignItems="center" spacing={0.5}>
              <Box
                sx={{
                  ...
                  height: 12,
                  borderRadius: '50%',
                  bgcolor: capitalDesignTokens.semanticColors.dataVisualization.sequence800,
                }}
              />
              <Typography variant="caption" color="text.secondary">
